...
jQuery.widget("ui.atk4_grid", {
	...
	_create: function(){
		...
		thead.find('a').click(function(ev){
			ev.preventDefault();
			self.element.atk4_load({0:jQuery(this).attr('href'),cut_object:self.element.attr('id')});
	...
	reload: function(){
		var url=this.base_url;
		url=jQuery.atk4.addArgument(url,'cut_object='+this.name);
	...
	reloadData: function(q){
		...
		var t=this;
		url=jQuery.atk4.addArgument(url,'cut_object='+this.name);
		url=jQuery.atk4.addArgument(url,'cut_region=grid_content');
		jQuery.each(q,function(k,v){
			url=jQuery.atk4.addArgument(url,escape(k)+'='+escape(v));
		});
