		...
		this.element.find('a').click(function(e){
			...
			if(self.options.submenus){
				if(self.options.submenu_aciton){
					...
					jQuery(this).closest('ul,.menu,.header_subnav').
					    find('.submenu-active').
			...
			*/
			...
			var t=this;
			jQuery(self.options.target).atk4_load(url,function(){
...
/*
        jQuery('.atk4-link').atk4_click(function(url){
            //jQuery.atk4.page(url);
			jQuery('#Content').atk4_load2(url);
		});
*/
...
*/
	...
	openPage: function(h,fn){
		...
		var el=this.element.find('a[href='+h+']');
		if(el.length){
			if(!fn)fn=jQuery.atk4.readyLast;
			jQuery.atk4.readyLast=undefined;
			el.closest('li').find('a').eq(0).click();
			menu.click(el,h,fn);
		}
	},
    ...
    click: function(el,url,fn){
        ...
        el.parent().parent().find('.current').each(function(){
            ...
            var next=jQuery(this).removeClass('current').filter('a').next();
    ...
    onClick: function(el,url,fn){
        if(this.options['callback']){
            ...
            if(url!='#')jQuery('#Content').atk4_load(url,fn);
        }
    }
});
