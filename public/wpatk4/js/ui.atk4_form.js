...
jQuery.widget("ui.atk4_form", {
	...
	_create: function(){
		...
		this.form.find('input').bind('keypress',function(e){
			if(jQuery(this).is('.ui-autocomplete-input'))return true;
			if(e.keyCode==13){
	   ...
	   */
		...
		this.form.find(':input').each(function(){
			if(jQuery(this).attr('type')=='checkbox')
				jQuery(this).attr('data-initvalue',jQuery(this).attr('checked'))
			else
				jQuery(this).attr('data-initvalue',jQuery(this).val())
			})
		.bind('change',function(ev){
				//if(jQuery(this).attr('type')=='checkbox')
			if(jQuery(this).attr('data-initvalue')==jQuery(this).val()){
				ev.preventDefault();
				return;
			}else {
				jQuery(this).attr('data-initvalue',jQuery(this).val());
		...
		this.form.find('input[type=submit]').each(function(){
			var a=jQuery('<a class="gbutton button_style1"></a>');
			var s=jQuery(this);
			a.text(s.attr('value'));
	...
	setFieldValue: function(field_name,value){
		var f=jQuery('#'+this.id+'_'+field_name);
		...
		if(f.hasClass('field_reference')){
			...
			this.reloadField(field_name,null,function(){
				var f=jQuery('#'+self.id+'_'+field_name);
				var opt=f.find('option[value='+value+']');
				if(opt.length){
					f.val(value).change().trigger('change_ref');
				}else{
	...
	reloadField: function(field_name,url,fn,notrigger,arg){
		var field_id = jQuery("#"+this.id+' [data-shortname="'+field_name+'"]').first().attr("id");
		...
		url=jQuery.atk4.addArgument(url,this.id+'_cut_field',field_id);
		var f=jQuery("#"+field_id);
	...
	fieldError: function(field_name,error){
		...
		var field=
			typeof(field_name)=='string'?
				jQuery('[data-shortname="'+field_name+'"]','#'+this.id):
					field_name;
		...
		if(!this.template['field_error'].length){
			...
			jQuery.univ().errorMessage(error);
			return;
		}
	...
	submitForm: function(btn){
		...
		// do request
		jQuery.atk4.get(properties,params,function(res){
			var c=form._getChanged();
			form._setChanged(false);

			if(!jQuery.atk4._checkSession(res))return;
			/*
